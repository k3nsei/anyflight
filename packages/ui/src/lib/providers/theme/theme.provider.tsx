...
export const AnyflightUiThemeProvider = ({ theme: providedTheme = emptyTheme, children }: ThemeProviderProps) => {
  const { prefix } = useAnyflightUiConfig();
  const originTheme = useAnyflightUiTheme();
  const nestLevel = useAnyflightUiThemeNestLevel();

  const theme: AnyflightUiTheme = useMemo(() => merge({}, originTheme, providedTheme), [originTheme, providedTheme]);

  return (
    <AnyflightUiThemeNestLevelProvider nestLevel={nestLevel + 1}>
      <AnyflightUiThemeContext.Provider value={theme}>
        {nestLevel === 0 && (
          <Global
            styles={css`
